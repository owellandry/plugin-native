...
class BackpackListener(private val plugin: BackpackPlugin) : Listener {
    ...
    private fun updateBackpack(player: org.bukkit.entity.Player) {
    ...
    fun onPlayerMove(event: PlayerMoveEvent) {
        ...
        if (from.x == to.x && from.y == to.y && from.z == to.z
            && from.yaw == to.yaw && from.pitch == to.pitch) return
    ...
    fun onInventoryClick(event: InventoryClickEvent) {
        ...
        Bukkit.getScheduler().runTask(plugin, Runnable {
            updateBackpack(event.whoClicked as? org.bukkit.entity.Player ?: return@Runnable)
        })
    }
